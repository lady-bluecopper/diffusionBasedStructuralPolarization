...
class MCMC_LA:
    ...
    def MCMC_step(self,
        ...
        if self.node_labels[u] != self.node_labels[y] and self.node_labels[v] != self.node_labels[x]:
            # line 6 Algorithm 1: swap would change the JLM
            swapped[0] = -1
            return -2
        if v == x or u == y:
            # assert sorted([sorted([u, v]), sorted([x, y])]) == sorted([sorted([u, x]), sorted([v, y])])
